...
import com.defitech.GestUni.enums.BEDJRA.ArmelFilliereDto;
...
public class FiliereServices {
    ...
    public List<FiliereDto> getAllFilieres() {
        ...
        return filieres.stream().map(filiere -> {
            FiliereDto dto = new FiliereDto();
            dto.setId(filiere.getFiliereId());
            dto.setNomFiliere(filiere.getNomFiliere());
            dto.setDescription(filiere.getDescription());
            dto.setParcoursNom(filiere.getParcours().getNomParcours());
            return dto;
    ...
    public List<ArmelFilliereDto> findByParcoursId(Long parcoursId) {
        List<Filiere> filieres = filiereRepository.findByParcoursParcoursId(parcoursId);
        return filieres.stream()
                .map(f -> new ArmelFilliereDto(f.getFiliereId(), f.getNomFiliere(), f.getDescription()))
                .collect(Collectors.toList());
    }
...
//////////////////////////////////////////////////NEW///////////////////////////////////
    ...
    public List<UEDto> getCoursByFiliereId(Long filiereId) {
        Filiere filiere = filiereRepository.findById(filiereId).orElse(null);
        if (filiere != null) {
            return filiere.getUe().stream()
                    .map(ueService::convertToDTO)
                    .collect(Collectors.toList());
        }
    ...
    public List<FiliereDto> getFilieresDTOByParcours(Long parcoursId) {
        List<Filiere> filieres = filiereRepository.findByParcours_ParcoursId(parcoursId);
        return filieres.stream().map(this::mapToDto).collect(Collectors.toList());
    }
